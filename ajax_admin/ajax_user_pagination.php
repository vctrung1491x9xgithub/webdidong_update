...
if (isset($_POST['per_page'])  && isset($_POST['page']) && isset($_POST['userPoint'])  && isset($_POST['txtSearch'])) {
    ...
    // TÌM KIẾM THEO TÊN VÀ ThÀNH PHỐ
    ...
    $users = mysqli_query($conn, $users_select);
    $users =  $users->num_rows;
    $totalPages = ceil($users / $item_per_page);
    $users = mysqli_query($conn, $page_user);
    if ($users->num_rows > 0) {
        $stt = $current_page; ?>
