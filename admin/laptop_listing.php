...
?>
    <div class="framework ">
        <div class="breadcrumb bg-white p-2 m-0 mb-3 shadow-sm">
            <h1><i class="ti-home text-secondary"></i><span class="mx-2">&#10095;</span><a href=""> Quản lý sản phẩm</a><span class="mx-2">&#10095;</span><span class="font-weight-bold">Danh sách laptop</span></h1>
        </div>
        ...
        <div class="table-responsive-lg p-2 bg-white shadow-sm border border-success" id="tbl-phone-listing">
            ...
            $totalRecords = mysqli_query($conn, "SELECT * FROM san_pham WHERE id_mat_hang = 'LAPTOP'");
            $totalRecords = $totalRecords->num_rows;
            $totalPages = ceil($totalRecords / $item_per_page);
            $products = mysqli_query($conn, "   SELECT *
                                                 FROM san_pham as sp, danh_muc_thuong_hieu as dmth
                                                 WHERE sp.id_thuong_hieu = dmth.id_thuong_hieu and sp.id_mat_hang = 'LAPTOP'  
                                                         ORDER BY id_san_pham DESC LIMIT " . $item_per_page . " OFFSET " . $offset);
            if ($products->num_rows > 0) { ?>
                <table class="table table-hover"">
                    <caption class="pb-2 border-bottom mb-2 font-weight-bold">Danh sách điện thoại </caption>
                        ...
                        ?>
                            <tr>
                                ...
                                <td>
                                    ...
                                    <a class="btn btn-sm btn-danger" href="./laptop_listing.php?action=del&id=<?= $row['id_san_pham'] ?>" onclick="return confirm('Bạn muốn xoá sản phẩm này này?')">
                ...
                <?php
                include '../pagination/laptop_pagination.php';
                ?>
            <?php  } else {
